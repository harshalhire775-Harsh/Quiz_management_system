...
const ManageDepartments = () => {
    ...
    const [admins, setAdmins] = useState([]); // This state seems unused based on the provided snippet, but keeping it as it was there.
    ...
    // ... (keep existing helper functions like fetchHods, handleDeleteHod, handleBlockHod, fetchDepartments, generateCredentials)

    // REDEFINING THE FUNCTIONS TO AVOID SKIPPING CODE, BUT I WILL FOCUS ON THE CHANGES
    // TO MINIMIZE TOKEN USAGE, I WILL USE MULTIPLE REPLACEMENTS IF NEEDED OR A TARGETED BLOCK.
    // DUE TO THE NATURE OF 'replace_file_content', I CANNOT SKIP CHUNKS EASILY IF THEY ARE INTERTWINED.
    // BUT 'successMsg' state is defined near line 125.

    // Let's target the handleCreate and state definition area.

    const openCreateModal = () => {
        if (isSuperAdmin) {
            const { id, pass } = generateCredentials();
            // Auto-generate email for backend compatibility (Super Admin)
    ...
    const handleCreate = async (e) => {
        ...
        try {
            const payload = {
                ...newDept,
                // If it's Super Admin, ensure hodName is set if empty (though we handle this in backend too usually, but safe to keep)
                hodName: isSuperAdmin ? (newDept.name ? `${newDept.name} Admin` : 'College Admin') : newDept.hodName
            };
            ...
            let msg = '';
            if (error.response?.status === 404 && !isSuperAdmin) {
                msg = 'Your account is not linked to any College. Please contact Super Admin.';
            } else {
                msg = error.response?.data?.message || 'Failed to create department';
            }
    ...
    return (
        <div className="p-8 bg-slate-50 min-h-screen animate-fade-in relative">
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    ...
                    >
                        <h3 className="text-2xl font-black text-slate-800 mb-6 flex items-center gap-2">
                            <Building2 size={24} className="text-blue-600" /> {isSuperAdmin ? 'New College' : 'New Department'}
                        </h3>
                        <form onSubmit={handleCreate} className="space-y-6">
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">{isSuperAdmin ? 'College Name' : 'Department Name'}</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder={isSuperAdmin ? "e.g. Government Science College" : "e.g. Computer Science"}
                                        required
                                        autoFocus
                                    />
                                </div>
                            ...
                            {/* --- SUPER ADMIN VIEW: Auto-Generated Credentials --- */}
                            {isSuperAdmin ? (
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                    <div>
                        ...
                        <h2 className="text-3xl font-black text-slate-800 flex items-center gap-3">
                            <Building2 size={32} className="text-blue-600" /> {isSuperAdmin ? 'Manage College' : 'Manage Departments'}
                        </h2>
                    </div>
                    ...
                    <div className="flex gap-3">
                        <div className="bg-white px-4 py-2.5 rounded-xl border border-slate-200 flex items-center gap-3 shadow-sm focus-within:ring-2 focus-within:ring-blue-500/20 transition-all w-full md:w-auto">
                            ...
                            <input
                                type="text"
                                placeholder={isSuperAdmin ? "Search Colleges..." : "Search Departments..."}
                                value={searchTerm}
                        ...
                        >
                            <Plus size={18} />
                            <span>{isSuperAdmin ? 'Add College' : 'Add Department'}</span>
                        </button>
                    </div>
                </div>
                    ...
                    {filteredDepts.map((dept) => (
                        ...
                        >
                            ...
                            <div className="p-5 flex-1 flex flex-col">
                                ...
                                {/* Main Info Section - For BOTH Super Admin and Admin (HOD) */}
                                        ...
                                        <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 relative group/creds hover:border-blue-200 transition-colors">
                                            <div className="flex items-center gap-3 mb-3 pt-2">
                                                <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-sm">
                                                    {/* Handle both hod object (Colleges) and direct email/name (Subjects) */}
                                                    {(dept.hod?.name || dept.email?.split('@')[0] || "U").charAt(0).toUpperCase()}
                                                </div>
                                                <div>
                                                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">
                                                        {isSuperAdmin ? 'Administrator' : 'Head/Teacher'}
                                                    </p>
                ...
                {/* HOD Directory Section - Only for Super Admin */}
                {isSuperAdmin && (
                    <div className="mt-16">
